...
if __name__ == '__main__':
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"Epoch [{epoch + 1}/{EPOCHS}], Loss: {loss.item():.6f}, BER: {current_ber:.6f}, FER: {current_fer:.6f}")
    print("--- 훈련 종료 ---")
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(date,ber_data[0],label="irregular")
    plt.legend(loc='upper right')
    plt.xlabel("Epoch")
    plt.ylabel("Bit Error Rate (BER)")
    plt.title("BER (N0576_R34_z24 - irregular)")
   # legends = [f"learning rate={lr[i]}, SNR={SNR_DB}" for i in range(len(lr))]
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(date,fer_data[0],label="irregular")
    plt.legend(loc='upper right')
    plt.xlabel("Epoch")
    plt.ylabel("Frame Error Rate (FER)")
    plt.title("FER (N0576_R34_z24 - irregular)")
   # legends = [f"learning rate={lr[i]}, SNR={SNR_DB}" for i in range(len(lr))]
    plt.legend()
    plt.grid(True, which="both", ls="--")
    plt.show()